...
public class PhisiX {
    ...
    public static void main(String[] args) {
        ...
        Vector vector = new Vector(8, -50, false);
        ball = new P_Ball(engine.getLevel(), 10, 300, vector);
        engine.start();
    ...
    public static void drawTrail(Screen screen) {
        ...
        for (Map.Entry<Integer, Integer> entry : trail.entrySet())
            screen.renderSprite(entry.getKey() - (ball.getSprite().getWidth() / 2), entry.getValue() - (ball.getSprite().getHeight() / 2), new Sprite(1, 1, 0x000000), true);
